 ...
 *
 * Last modified: 2021.06.25 at 20:38
 */
...
class FaqListTable implements ConfigureTcaTableInterface
{
    /**
     * @inheritDoc
     */
    ...
    {
        $table->setTitle('faxBe.t.faq.list.title')
        ...
        $type->getTab(0)->addMultiple(static function () use ($type) {
            $type->getField('title')
                 ->setLabel('faxBe.t.faq.list.field.title')
                 ->applyPreset()->input(['maxLength' => 128]);
            
            $type->getField('items')
                 ->setLabel('faxBe.t.faq.list.field.items')
                 ->applyPreset()->inline(FaqItemTable::class, ['required']);
        });
    }
